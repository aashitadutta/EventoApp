...
public class UserEventDescription extends BaseActivity{
	TextView name,date,short_desc,org,society,phone,desc,venue,prize,name1,date1,short1,org1,phone1,desc1,venue1,prize1,society1;
	ImageView image;
	Button join,back;
...
alert_dialog.show();
	 ...
	 if(id1!=null){
		...
		if(rs.moveToFirst()){
			do{
				...
				event_names = c.getString(1);
			
				//
			}while(rs.moveToNext());
		}
		DBAdapter.close();
		 //Toast.makeText(getApplicationContext(), event_names, Toast.LENGTH_LONG).show(); 
		...
		phone.setText(event_contact);
		 ...
		 image_One = DBAdapter.retriveImgDetails(ename);
	...
	// image.setImageBitmap(image_One.getBitmap()); 
	  ...
	  // DBAdapter.open();
		...
		// Images image_One = DBAdapter.retriveImg(Integer.parseInt(id1));
			// image.setImageBitmap(image_One.getBitmap()); 
		
		/*SharedPreferences user=PreferenceManager.getDefaultSharedPreferences(UserEventDescription.this);
	    	String abc=user.getString("user_image",event_names);*/
		
		
		
		/*if(ename!=null){
			 DBAdapter.open();
			 Cursor cursor = DBAdapter.getEventData(ename);
